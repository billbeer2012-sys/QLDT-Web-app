...
/**
 * Kiểm tra sinh viên trùng lịch học
 * @param {Object} pool - Database pool
 * @param {string} MaLHP - Mã lớp học phần hiện tại
 * @param {string} Ngay - Ngày học
 * @param {number} TietStart - Tiết bắt đầu
 * @param {number} TietEnd - Tiết kết thúc
 * @param {string} excludeCondition - Điều kiện loại trừ (cho chế độ edit)
 * @returns {Object} { hasOverlap: boolean, conflicts: [{count, Tenlop, Viettat}] }
 */
const checkStudentOverlap = async (pool, MaLHP, Ngay, TietStart, TietEnd, excludeCondition = '') => {
    try {
        const request = new sql.Request(pool);
        request.input('MaLHP', sql.NVarChar, MaLHP);
        ...
        // Bước 1: Tìm các lớp có lịch trùng thời gian (cùng ngày, trùng khoảng tiết)
        const conflictingClassesQuery = `
            SELECT DISTINCT TKB.MaLHP
            FROM TKB 
            WHERE TKB.Ngay = @Ngay 
        ...
        const conflictingResult = await request.query(conflictingClassesQuery);

        if (conflictingResult.recordset.length === 0) {
            return { hasOverlap: false, conflicts: [] };
        }

        // Bước 2: Lấy danh sách MaLHP bị trùng thời gian
        const conflictingMaLHPs = conflictingResult.recordset.map(r => r.MaLHP);

        // Bước 3: Tìm sinh viên trùng giữa lớp hiện tại và các lớp bị trùng thời gian
        // Sử dụng INTERSECT để tìm nhanh
        const overlapQuery = `
            SELECT 
                lhp.Tenlop, 
                hp.Viettat,
                COUNT(DISTINCT sv_conflict.MaSV) as StudentCount
            FROM SinhvienLopHP sv_conflict
            JOIN LopHP lhp ON sv_conflict.MaLHP = lhp.MaLHP
            JOIN Hocphan hp ON lhp.MaHP = hp.MaHP
            WHERE sv_conflict.MaLHP IN (${conflictingMaLHPs.map((_, i) => `@ConflictLHP${i}`).join(',')})
              AND sv_conflict.MaSV IN (
                  SELECT MaSV FROM SinhvienLopHP WHERE MaLHP = @MaLHP
              )
            GROUP BY lhp.MaLHP, lhp.Tenlop, hp.Viettat
            HAVING COUNT(DISTINCT sv_conflict.MaSV) > 0
        `;

        // Thêm input cho các MaLHP bị trùng
        conflictingMaLHPs.forEach((maLHP, i) => {
            request.input(`ConflictLHP${i}`, sql.NVarChar, maLHP);
        });

        const overlapResult = await request.query(overlapQuery);

        if (overlapResult.recordset.length === 0) {
            return { hasOverlap: false, conflicts: [] };
        }

        // Trả về danh sách các lớp có sinh viên trùng
        const conflicts = overlapResult.recordset.map(r => ({
            count: r.StudentCount,
            Tenlop: r.Tenlop,
            Viettat: r.Viettat
        }));

        return { hasOverlap: true, conflicts };
    } catch (error) {
        console.error('Lỗi khi kiểm tra trùng sinh viên:', error);
        // Nếu lỗi, cho phép tiếp tục lưu
        return { hasOverlap: false, conflicts: [] };
    }
};
...
module.exports = function (poolPromise) {
        ...
        try {
            ...
            const query = `
               SELECT 
                   ...
                   ISNULL(ISNULL(hv.Viettat + N' ', N'') + gv.Holot + N' ' + gv.Ten, 'N/A') as HoTenGV,
                   ph.Tenphong,
                   np.Nhomphong,
                   hp.Viettat as TenHP,
                   lhp.Tenlop,
    ...
    // BỔ SUNG: API lấy danh sách nhóm phòng
    router.get('/nhomphong', async (req, res) => {
        try {
            const pool = await poolPromise;
            const result = await pool.request().query(`SELECT MaNP, Nhomphong FROM Nhomphong ORDER BY Nhomphong`);
            res.json(result.recordset);
        } catch (err) {
    ...
    // CẬP NHẬT: Bổ sung tham số maNP để lọc phòng theo nhóm phòng
    router.get('/resources', async (req, res) => {
        const { ngay, tietStart, soTiet, excludeMaLHP, excludeTiet, maNP } = req.query;
        ...
        try {
            ...
            // BỔ SUNG: Điều kiện lọc theo nhóm phòng
            let nhomphongCondition = '';
            if (maNP && maNP !== 'all') {
                request.input('MaNP', sql.NVarChar, maNP);
                nhomphongCondition = 'AND Phonghoc.MaNP = @MaNP';
            }
            ...
            const [teachersResult, roomsResult] = await Promise.all([
                ...
                request.query(`
                   ...
                   ) ${nhomphongCondition} ORDER BY Tenphong
               `)
            ]);
    ...
    const handleSaveRequest = async (req, res, mode) => {
        ...
        try {
            ...
            // BỔ SUNG: Kiểm tra trùng sinh viên (trừ khi forceOverlap = true)
            if (!forceOverlap) {
                const Tiet = parseInt(entryData.Tiet, 10);
                const Sotiet = parseInt(entryData.Sotiet, 10);
                const TietEnd = Tiet + Sotiet - 1;
                ...
                let excludeCondition = '';
                if (mode === 'edit' && entryData.originalData) {
                    // Loại trừ record đang sửa
                    excludeCondition = `AND NOT (TKB.MaLHP = '${entryData.MaLHP}' AND TKB.Ngay = '${entryData.originalData.Ngay}' AND TKB.Tiet = ${entryData.originalData.Tiet})`;
                }

                const overlapResult = await checkStudentOverlap(
                    pool,
                    entryData.MaLHP,
                    entryData.Ngay,
                    Tiet,
                ...
                if (overlapResult.hasOverlap) {
                    // Trả về thông tin trùng sinh viên để frontend hiển thị cảnh báo
            ...
            console.error('Lỗi handleSaveRequest:', err);
    ...
    router.post('/tkb/batch', async (req, res) => {
        ...
        try {
            ...
            // BỔ SUNG: Kiểm tra trùng sinh viên cho tất cả entries
            if (!forceOverlap) {
                const allConflicts = [];
                for (const entry of entries) {
                    const Tiet = parseInt(entry.Tiet, 10);
                    const Sotiet = parseInt(entry.Sotiet, 10);
                    const TietEnd = Tiet + Sotiet - 1;

                    const overlapResult = await checkStudentOverlap(pool, MaLHP, entry.Ngay, Tiet, TietEnd, '');

                    if (overlapResult.hasOverlap) {
                        // Gộp conflicts, tránh trùng lặp
                        for (const conflict of overlapResult.conflicts) {
                            const existing = allConflicts.find(c => c.Tenlop === conflict.Tenlop);
                            if (existing) {
                                existing.count = Math.max(existing.count, conflict.count);
                            } else {
    ...
    // BỔ SUNG: API lấy lịch thi theo giảng viên cho Schedule Builder
    router.get('/exam-schedule-by-teacher', async (req, res) => {
        const { maGV, startDate, endDate } = req.query;

        if (!maGV || !startDate || !endDate) {
            return res.status(400).json({ message: "Thiếu tham số maGV, startDate hoặc endDate." });
        }
        ...
        try {
            ...
            // Query tất cả các lịch thi từ 3 bảng Phongthi0/1/2 mà GV là cán bộ coi thi
            const result = await request.query(`
                WITH PhongthiAll AS (
                    SELECT 
                        pt0.MaPT0 AS MaPTX, 
                        ...
                        pt0.MaGV1, pt0.MaGV2, pt0.MaPH, pt0.Ghichu, pt0.Phongthi
                    FROM Phongthi0 pt0
                    WHERE pt0.Ngay BETWEEN @StartDate AND @EndDate
                      AND (pt0.MaGV1 = @MaGV OR pt0.MaGV2 = @MaGV)
                    UNION ALL
                    SELECT 
                        pt1.MaPT1 AS MaPTX, 
                        'L1' AS Lanthi, 
                        ...
                        pt1.MaGV1, pt1.MaGV2, pt1.MaPH, pt1.Ghichu, pt1.Phongthi
                    FROM Phongthi1 pt1
                    WHERE pt1.Ngay BETWEEN @StartDate AND @EndDate
                      AND (pt1.MaGV1 = @MaGV OR pt1.MaGV2 = @MaGV)
                    UNION ALL
                    SELECT 
                        pt2.MaPT2 AS MaPTX, 
                        'L2' AS Lanthi, 
                        ...
                        pt2.MaGV1, pt2.MaGV2, pt2.MaPH, pt2.Ghichu, pt2.Phongthi
                    FROM Phongthi2 pt2
                    WHERE pt2.Ngay BETWEEN @StartDate AND @EndDate
                      AND (pt2.MaGV1 = @MaGV OR pt2.MaGV2 = @MaGV)
                )
                SELECT
                    pta.MaPTX, pta.Lanthi, pta.Ngay, pta.Gio, pta.Phut, pta.Thoigian, pta.Ghichu, pta.Phongthi,
                    hp.Hocphan AS TenHocPhan,
                    ph.Tenphong AS DiaDiemThi,
                    ISNULL(hv1.Viettat + ' ', '') + cb1.Holot + ' ' + cb1.Ten AS CBCoiThi1,
                    ISNULL(hv2.Viettat + ' ', '') + cb2.Holot + ' ' + cb2.Ten AS CBCoiThi2
                FROM PhongthiAll pta
                LEFT JOIN Hocphan hp ON pta.MaHP = hp.MaHP
                LEFT JOIN Phonghoc ph ON pta.MaPH = ph.MaPH
                LEFT JOIN Giaovien cb1 ON pta.MaGV1 = cb1.MaGV
                LEFT JOIN Hocvi hv1 ON cb1.MaHV = hv1.MaHV
                LEFT JOIN Giaovien cb2 ON pta.MaGV2 = cb2.MaGV
                LEFT JOIN Hocvi hv2 ON cb2.MaHV = hv2.MaHV
                ORDER BY pta.Ngay, pta.Gio, pta.Phut
            `);

            res.json(result.recordset);

        } catch (err) {
            console.error("API Get Exam Schedule by Teacher Error:", err);
