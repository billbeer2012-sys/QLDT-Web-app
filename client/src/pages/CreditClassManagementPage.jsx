 ...
 * Cập nhật: 07/02/2026
 * Tóm tắt:
 * - Thêm nút "Tạo phòng thi" bên cạnh nút "Nhập điểm thi" cho các học phần có trạng thái "Đã có điểm thi".
 * - Bổ sung "Xếp lịch thi"
 */
...
// BỔ SUNG 05/02/2026: Import useTabStore để mở tab ExamRoomBuilderPage
...
// BỔ SUNG: `DataTable` nhận thêm `studentPositions`, `selectedNode`, `onOpenExamRoomBuilder`
const DataTable = ({ columns, data, title, uniqueKeyField, sortConfig, onSort, contentType, onStudentsDelete, onStudentsSort, onRowDoubleClick, pendingChanges, setPendingChanges, visibleColumns, onVisibleColumnsChange, evaluationForms, studentPositions, activePreset, onPresetChange, selectedNode, onOpenExamRoomBuilder }) => {
            ...
            <div className="flex-shrink-0">
                ...
                <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                    <div className="flex items-center gap-2">
                        ...
                        {/* BỔ SUNG 05/02/2026: Nút Thi học phần - Chỉ hiển thị khi Admin/Khaothi chọn Cấp 4 (Học phần) */}
                        {contentType === 'classes' && canUpdateClasses && selectedNode?.level === 4 && onOpenExamRoomBuilder && (
                            <DropdownMenu.Root>
                                <DropdownMenu.Trigger asChild>
                                    <button className="flex items-center gap-2 px-3 py-2 border rounded-md text-white bg-purple-600 dark:bg-purple-700 hover:bg-purple-700 dark:hover:bg-purple-600 text-sm">
                                        <FileCheck2 className="w-4 h-4" />
                                ...
                                <DropdownMenu.Portal>
                                    <DropdownMenu.Content sideOffset={5} className="w-48 bg-white dark:bg-gray-800 rounded-md shadow-lg p-2 border dark:border-gray-700 z-50">
                                        <DropdownMenu.Item
                                            onSelect={() => onOpenExamRoomBuilder('exam-room')}
                                            className="p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer outline-none text-sm"
                                        >
                                            Tạo phòng thi
                                        </DropdownMenu.Item>
                                        <DropdownMenu.Item
                                            onSelect={() => onOpenExamRoomBuilder('exam-schedule')}
                                            className="p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer outline-none text-sm"
                                        >
                                            Xếp lịch thi
                                        </DropdownMenu.Item>
...
const CreditClassManagementPage = () => {
    ...
    // BỔ SUNG 05/02/2026: Handler mở tab Tạo phòng thi / Xếp lịch thi
    const { openTab } = useTabStore();
    const handleOpenExamRoomBuilder = useCallback((mode) => {
        if (!selectedNode || selectedNode.level !== 4) return;

        if (mode === 'exam-room') {
            openTab({
                id: `exam-room-builder-${selectedNode.params.maHP}`,
                title: `Tạo phòng thi - ${selectedNode.name}`,
                componentName: 'ExamRoomBuilderPage',
                props: {
                    maHK: selectedNode.params.maHK,
                    maDV: selectedNode.params.maDV,
                    maHP: selectedNode.params.maHP,
                    tenHP: selectedNode.name
                }
            });
        } else if (mode === 'exam-schedule') {
            openTab({
                id: `exam-schedule-${selectedNode.params.maHP}`,
                title: `Xếp lịch thi - ${selectedNode.name}`,
                componentName: 'ExamRoomSchedulePage',
                props: {
                    maHK: selectedNode.params.maHK,
                    maDV: selectedNode.params.maDV,
                    maHP: selectedNode.params.maHP,
            ...
            <div className="flex flex-col h-full">
                ...
                <div className="flex-grow overflow-hidden">
                    <DataTable
                        ...
                        // BỔ SUNG 05/02/2026: Props cho chức năng Thi học phần
                        selectedNode={selectedNode}
                        onOpenExamRoomBuilder={handleOpenExamRoomBuilder}
                    />
                </div>
            </div>
        );
