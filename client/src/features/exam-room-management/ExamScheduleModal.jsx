/*
 * Đường dẫn file: client/src/features/exam-room-management/ExamScheduleModal.jsx
...
// ========================================
// Component: DeleteConfirmDialog - Part 7.4
// ========================================
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full mx-4 p-6">
                <div className="flex items-center gap-3 text-red-600 mb-4">
                    <AlertTriangle className="w-6 h-6" />
                    <h3 className="text-lg font-semibold">Cảnh báo xóa dữ liệu</h3>
                </div>

                <p className="mb-6 text-gray-700 dark:text-gray-300">
                    Thông tin đã thiết lập Lịch coi thi bị xóa trống. Bạn có chắc chắn xóa không?
                </p>
...
const ExamScheduleModal = ({ isOpen, onClose, onSave, maPT, examType, roomData, maHK, maHP }) => {
    ...
    // Delete confirm dialog state - Part 7.4
    ...
    useEffect(() => {
        const fetchRoomGroups = async () => {
            try {
                ...
                // Task 8.2.1: Mặc định Nhóm phòng = '' (Tất cả)
            } catch (error) {
    ...
    // Đảm bảo fetch rooms khi form đã init và dependencies thay đổi
    useEffect(() => {
        if (isFormInitialized && ngay && gio && phut && thoigian) {
            fetchAvailableRooms();
        }
    }, [isFormInitialized, ngay, gio, phut, thoigian, maNP, fetchAvailableRooms]);
    ...
    // Handlers
    // ========================================
    // Part 7.4: Mở dialog xác nhận xóa
    const handleClear = () => {
        setShowDeleteDialog(true);
    };

    // Part 7.4: Xác nhận xóa
    ...
    const handleSave = async (forceSkipConflict = false) => {
        ...
        try {
            ...
            setLoading(false);
            // Không đóng ConflictDialog ở đây vì nó sẽ đóng dialog khi có conflict
        }
    };
    ...
    return (
        <>
            ...
            {/* Delete Confirm Dialog - Part 7.4 */}
            <DeleteConfirmDialog
